package com.suzhou.concept.lil.ui.ad.util.upload;
...
import com.suzhou.concept.lil.ui.ad.util.upload.submit.AdLocalMarkBean;
import com.suzhou.concept.lil.ui.ad.util.upload.submit.AdSubmitBean;
...
public class AdUploadManager {
     ...
     * 点击广告上传
     *
     * @param showType                  {@link AdShowUtil.NetParam 的AdShowPosition}
     * @param adType                    {@link AdShowUtil.NetParam 的AdType}
    ...
    public void clickAdForReward(String showType, String adType, OnAdClickCallBackListener onAdClickCallBackListener) {
        ...
        RetrofitUtil.getInstance().getAdClickReward(GlobalMemory.INSTANCE.getUserInfo().getUid(), showPositionType, adShowType)
    ...
    public void submitAdMsgData(String packageName, List<AdLocalMarkBean> localAdList, OnAdSubmitCallbackListener onAdSubmitCallbackListener) {
        ...
        //设备id
        TelephonyManager manager = (TelephonyManager) AppClient.context.getSystemService(Context.TELEPHONY_SERVICE);
        ...
        //请求数据
        RetrofitUtil.getInstance().submitAdData(GlobalMemory.INSTANCE.getUserInfo().getUid(), device, deviceId, packageName, submitAdData)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
