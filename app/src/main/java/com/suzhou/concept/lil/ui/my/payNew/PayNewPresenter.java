...
import com.suzhou.concept.activity.dollar.PayResult;
...
public class PayNewPresenter extends BasePresenter<PayNewView> {
    ...
    public void detachView() {
        super.detachView();
        LibRxUtil.unDisposable(payDis);
    }

    //获取支付宝的支付链接
    public void getAliPayOrderLink(String amount,String productId,String subject,String body,String price,long deduction){
        checkViewAttach();
        LibRxUtil.unDisposable(payDis);
        RetrofitUtil.getInstance().getAliPayLink(amount,price,productId,subject,body,deduction)
    ...
    private void showAliPay(String payOrderLink){
        ...
        Observable.create(new ObservableOnSubscribe<PayResult>() {
            ...
            public void subscribe(ObservableEmitter<PayResult> emitter) throws Exception {
                PayTask payTask = new PayTask((AppCompatActivity)getMvpView());
