...
import com.suzhou.concept.bean.AllVipItem
import com.suzhou.concept.bean.VipBean
import com.suzhou.concept.databinding.ActivityMemberCentreBinding
import com.suzhou.concept.lil.ui.my.payNew.PayNewActivity
import com.suzhou.concept.utils.ExtraKeysFactory
import com.suzhou.concept.utils.GlobalMemory
import com.suzhou.concept.utils.addSingleRadioButton
import com.suzhou.concept.utils.getSingleRadio
import com.suzhou.concept.utils.showGoLoginDialog
import com.suzhou.concept.utils.timeStampDate

class MemberCentreActivity : BaseActivity<ActivityMemberCentreBinding>() ,View.OnClickListener,TabLayout.OnTabSelectedListener{
    /**
     * 本应用VIP
     * */
    ...
    override fun ActivityMemberCentreBinding.initBinding() {
        setTitleText("会员中心")
        binding.activateNow.setOnClickListener(this@MemberCentreActivity)
        ...
        contentVip.apply {
            removeAllViews()
            if (intent.getBooleanExtra(ExtraKeysFactory.buyGoldVip,false)){
    ...
    private fun refreshLocalUserInfo(){
        binding.item=GlobalMemory.userInfo
        val listen=if (!GlobalMemory.isLogin()){
        ...
        GlobalMemory.userInfo.apply {
            binding.vipTime.text = showVipTime()
        }
    }
    ...
    private fun getVipContentText(vipType:Int=appVip):TextView{
        ...
        val params= with(LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,LinearLayout.LayoutParams.WRAP_CONTENT)){
            bottomMargin=5
            topMargin=5
            leftMargin=20
            rightMargin=20
    ...
    private fun getAllVip():LinearLayout{
        ...
        val list= mutableListOf(AllVipItem("无广告",R.drawable.tequan1),
            AllVipItem("尊贵标识",R.drawable.tequan2),
            AllVipItem("调节语速",R.drawable.tequan3))
        val groupParams=LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,LinearLayout.LayoutParams.WRAP_CONTENT)
        groupLayout.apply {
        ...
        list.apply {
            ...
            add(AllVipItem("查看解析",R.drawable.tequan5))
    ...
    private fun getAllVipRowLayout(list:List<AllVipItem>):LinearLayout{
        ...
        for (i in list.indices){
            val textParams= with(LinearLayout.LayoutParams(0,LinearLayout.LayoutParams.WRAP_CONTENT,1f)){
                setMargins(5)
                this
            }
...
//                    startPayActivity(array[0],array[1])
                    ...
                    //获取必要的数据
                    val firstIndex = array[0].indexOf("会员")
                    val lastIndex = array[0].indexOf("个月")
                    val amount = array[0].substring(firstIndex+2,lastIndex)
                    val price = array[1].toString()
                    var subject = "本应用会员"
                    var productId = "10"
                    if (binding.tabLayout.selectedTabPosition == appVip){
                        subject = "本应用会员"
                        productId = "10"
                    }else if (binding.tabLayout.selectedTabPosition == allVip){
                        subject = "全站会员"
                        productId = "0"
                    }else if (binding.tabLayout.selectedTabPosition == goldVip){
                        subject = "黄金会员"
                        productId = "21"
                    }
                    PayNewActivity.start(this,
                        PayNewActivity.PayType_vip,amount,price,subject,productId)
                }
            }
        }
    }
    ...
    //获取会员信息
    private fun showVipTime():String{
        var vipTime = 0L
        if (GlobalMemory.userInfo.expireTime.isNotEmpty() && GlobalMemory.userInfo.isVip()){
            vipTime = GlobalMemory.userInfo.expireTime.toLong()*1000
        }

        if (vipTime>0){
            return "会员到期：${vipTime.timeStampDate("yyyy-MM-dd")}"
        }else{
            if (GlobalMemory.userInfo.isVip()){
                return GlobalMemory.userInfo.getUserType()
            }
            return ""
        }
    }
