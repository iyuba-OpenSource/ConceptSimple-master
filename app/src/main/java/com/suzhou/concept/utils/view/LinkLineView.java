...
//        Collections.sort(rightList, (o1, o2) -> o1.getRow() - o2.getRow());
        ...
        // TODO: 2023/10/13 看前面没有进行混乱排序，这里处理下
        ...
        if (isImageType) {
            ratioW = 400 / 1080.0f;
            cellWidth = (int) (OtherUtils.INSTANCE.getScreenW(context) * ratioW);
            cellHeight = (int) (cellWidth * 280 / 400.0f);
        } else { // TextView类型
            ratioW = 400 / 1080.0f;
            cellWidth = (int) (OtherUtils.INSTANCE.getScreenW(context) * ratioW);
            cellHeight = (int) (cellWidth * 160 / 400.0f);
        }
        marginLeft = 0;
        marginRight = 0;
        marginBottom = OtherUtils.INSTANCE.dip2px(context, 15);
    ...
    public void justShowResult(List<LinkDataBean> linkDataBeanList) {
        ...
        this.post(() -> {
            ...
            for (int i = 0; i < resultList.size(); i++) {
                // 改变连线的颜色
                resultList.get(i).setColorString(LinkLineBean.COLOR_BLUE);
                newLinkLineBeanList.add(resultList.get(i));
                // 改变边框的颜色
                leftTvs.get(i).setBackground(context.getResources().getDrawable(newLinkLineBeanList.get(i).isRight() ? R.drawable.bg_link_line_green : R.drawable.bg_link_line_red));
                if (leftTvs.get(i) instanceof RoundedImageView) {
                    ((RoundedImageView) leftTvs.get(i)).setBorderColor(Color.BLACK);
                }
                rightTvs.get(i).setBackground(context.getResources().getDrawable(newLinkLineBeanList.get(i).isRight() ? R.drawable.bg_link_line_green : R.drawable.bg_link_line_red));
    ...
    private void verifyResult() {
        ...
        if (onChoiceResultListener != null&&!newLinkLineBeanList.isEmpty()) {
            onChoiceResultListener.onResultSelected(isRight, newLinkLineBeanList);
        }
    }
    ...
    private List<LinkLineBean> getResultList() {
        ...
        for (int i = 0; i < leftTvs.size(); i++) {
            ...
            for (int j = 0; j < rightList.size(); j++) {
                if (leftBean.getNum() == rightList.get(j).getNum()) {
    ...
    private RoundedImageView generateImageView(LinkDataBean bean) {
        ...
        riv.setCornerRadius(OtherUtils.INSTANCE.dip2px(context, 10));
        riv.setBorderWidth(OtherUtils.INSTANCE.dip2px(context, 2) * 1.0f);
        ...
        Glide.with(context).load(bean.getContent()).into(riv);
        return riv;
    }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        for (LinkLineBean item : linkLineBeanList) {
            if (item != null) {
                ...
                paint.setStrokeWidth(OtherUtils.INSTANCE.dip2px(context, 2));
    ...
    public interface OnChoiceResultListener {
        void onResultSelected(boolean correct, List<LinkLineBean> result);
    }

    public List<LinkDataBean> getDataList(){
        ArrayList<LinkDataBean> dataList = new ArrayList<>();
        for (int i = 0; i < allList.size(); i++) {
            LinkDataBean dataBean = allList.get(i);
            if (dataBean.getCol()==0){
    ...
    //获取左侧的数据
    public List<LinkDataBean> getLeftData(){
        return leftList;
    }

    //获取右侧的数据
    public List<LinkDataBean> getRightData(){
        return rightList;
    }
}
